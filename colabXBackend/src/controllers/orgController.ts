...
export async function updateOrganization(
    ...
    try {
        ...
        if (req.membership.role !== "admin") {
            res.status(403).json({ error: "Only admins can update the organization" });
            return;
        }
        ...
        if (name) {
            ...
            const [existing] = await db
                .select()
                .from(organization)
                .where(and(eq(organization.slug, slug), ne(organization.id, req.org.id)))
                .limit(1);
...
export async function deleteOrganization(
    ...
    try {
        ...
        if (req.membership.role !== "admin") {
            res.status(403).json({ error: "Only admins can delete the organization" });
            return;
        }
